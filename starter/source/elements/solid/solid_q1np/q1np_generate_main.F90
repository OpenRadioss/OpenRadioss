...
! ----------------------------------------------------------------------------------------------------------------------
          ...
          integer, dimension(:), allocatable, intent(inout) :: ikine          !< kinematic condition array (5*numnod)
...
!    integer, parameter :: surf_id_b = 2000
          ...
          integer, dimension(:), allocatable :: ikine_tmp
...
!           Reallocate kinematic arrays after control point promotion
              ...
              call my_alloc(ikine_tmp, 5*numnod_est, "IKINE_TMP", stat=stat)
              if(stat /= 0) call ancmsg(msgid=268,anmode=aninfo, &
                msgtype=msgerror, c1='IKINE_TMP')
              ikine_tmp(1:5*numnod_old) = ikine(1:5*numnod_old)
              ikine_tmp(5*numnod_old+1:5*numnod_est) = 0
              call my_dealloc(ikine)
              call move_alloc(ikine_tmp, ikine)
!
