...
!||====================================================================
        subroutine sigeps88(                                                     &
          ...
          asrate ,epsd    ,nvartmp ,vartmp  ,dmg    ,ngl     )
!-----------------------------------------------
!   M o d u l e s
!-----------------------------------------------
...
!----------------------------------------------------------------
          ...
          real(kind=WP) :: ldav,scale,lam_r,denom,lam_tg,prev,xhat,xmag,         &
            amax,dlam_eff,gdir,lamj,Rblend
          ...
          real(kind=WP), dimension(3,mvsiz) :: evv
          real(kind=WP), dimension(6,mvsiz) :: eps
...
!
          ...
          do i = 1,nel
            ...
            eps(2,i) = epsyy(i)
            eps(3,i) = epszz(i)
            eps(4,i) = epsxy(i)*half
            eps(5,i) = epsyz(i)*half
            eps(6,i) = epszx(i)*half
          enddo
!
          !< Compute principal strains and directions
          call valpvecdp(eps ,evv ,dirprv ,nel)
!
          ...
          do i = 1,nel
            ...
            epsp(1:3,1:3) = matmul(epsp(1:3,1:3),dirprv(1:3,1:3,i))
            epsp(1:3,1:3) = matmul(transpose(dirprv(1:3,1:3,i)),epsp(1:3,1:3))
            evvp(i,1) = epsp(1,1)
            evvp(i,2) = epsp(2,2)
            evvp(i,3) = epsp(3,3)
          enddo
!
          ...
          elseif (ismstr == 10 .or. ismstr == 12) then
            ...
            do i = 1, nel
              lam(i,1) = sqrt(evv(1,i) + one)
              lam(i,2) = sqrt(evv(2,i) + one)
              lam(i,3) = sqrt(evv(3,i) + one)
            enddo
          else
...
!
          ...
          if (rtype == 1) then
            ...
            do i = 1, nel
              ...
              if (ismstr == 0 .or. ismstr == 2 .or. ismstr == 4) then
                eep(i,1) = exp(evv(1,i))*evvp(i,1)
                eep(i,2) = exp(evv(2,i))*evvp(i,2)
                eep(i,3) = exp(evv(3,i))*evvp(i,3)
              !< Green-Lagrange strains
              elseif (ismstr == 10 .or. ismstr == 12) then
                eep(i,1) = half*(one/sqrt(evv(1,i) + one))*evvp(i,1)
                eep(i,2) = half*(one/sqrt(evv(2,i) + one))*evvp(i,2)
                eep(i,3) = half*(one/sqrt(evv(3,i) + one))*evvp(i,3)
              !< Engineering strains
              else
                eep(i,1) = evvp(i,1)
                eep(i,2) = evvp(i,2)
...
!
            ...
            signxx(i) = dirprv(1,1,i) * dirprv(1,1,i) * t(i,1) +                 &
                        dirprv(1,2,i) * dirprv(1,2,i) * t(i,2) +                 &
                        dirprv(1,3,i) * dirprv(1,3,i) * t(i,3)
            signyy(i) = dirprv(2,1,i) * dirprv(2,1,i) * t(i,1) +                 &
                        dirprv(2,2,i) * dirprv(2,2,i) * t(i,2) +                 &
                        dirprv(2,3,i) * dirprv(2,3,i) * t(i,3)
            signzz(i) = dirprv(3,1,i) * dirprv(3,1,i) * t(i,1) +                 &
                        dirprv(3,2,i) * dirprv(3,2,i) * t(i,2) +                 &
                        dirprv(3,3,i) * dirprv(3,3,i) * t(i,3)
            signxy(i) = dirprv(1,1,i) * dirprv(2,1,i) * t(i,1) +                 &
                        dirprv(1,2,i) * dirprv(2,2,i) * t(i,2) +                 &
                        dirprv(1,3,i) * dirprv(2,3,i) * t(i,3)
            signyz(i) = dirprv(2,1,i) * dirprv(3,1,i) * t(i,1) +                 &
                        dirprv(2,2,i) * dirprv(3,2,i) * t(i,2) +                 &
                        dirprv(2,3,i) * dirprv(3,3,i) * t(i,3)
            signzx(i) = dirprv(3,1,i) * dirprv(1,1,i) * t(i,1) +                 &
                        dirprv(3,2,i) * dirprv(1,2,i) * t(i,2) +                 &
                        dirprv(3,3,i) * dirprv(1,3,i) * t(i,3)
!
