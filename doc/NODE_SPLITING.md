...
if (new_crack > 0) then
  ...
  if (NSPMD > 1) then
    ...
    call merge_boundary_with_split(...)          ! keep startup sections + add new nodes
...
1. `spmd_rebuild_boundary` + `merge_boundary_with_split` — rebuild
   `NODES%BOUNDARY` / `BOUNDARY_ADD` (boundary node lists per rank), keeping the
   startup sections for pre-existing nodes and appending the new split nodes.
