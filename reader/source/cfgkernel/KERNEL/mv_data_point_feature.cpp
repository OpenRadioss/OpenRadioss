...
MvDataPointFeature_t::MvDataPointFeature_t(const string &name,
...
MvDataPointFeature_t::~MvDataPointFeature_t() {
}


/* --------- Output in an output stream --------- */

ostream &MvDataPointFeature_t::display(ostream &os,const MvDescriptor_t &descr,int level) const {
  for(int i=0;i<level;i++) os << "  ";
  display_props(os); 
  string arg_list = "";
  const vector<string>& argvect = getArgVect();  
  //
  os << "POINT(TITLE=\"" << getTitle() << "\""
     << ",KEYWORDS=("     
  ...
  if (!argvect.empty())
  {
      convert_vect_to_string(arg_list, argvect);
      os << arg_list;
  }
  os << "\")";
  return os;
