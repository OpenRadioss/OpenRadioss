...
// Char fortran -> c++
    ...
    cname[*S_ENTITY_TYPE]='\0';
//
    if (strncmp(cname,"MATERIAL",8) == 0 ) GlobalModelSDISelectionStartList("/MAT");
    if (strncmp(cname,"PROPERTY",8) == 0 ) GlobalModelSDISelectionStartList("/PROP");
    if (strncmp(cname,"PART",4) == 0 ) GlobalModelSDISelectionStartList("/PART");
    if (strncmp(cname,"TRANSFORM",9) == 0 ) GlobalModelSDISelectionStartList("/TRANSFORM");
    if (strncmp(cname,"SUBSET",6) == 0 ) GlobalModelSDISelectionStartList("/SUBSET");
    if (strncmp(cname,"FAILURE",7) == 0 ) GlobalModelSDISelectionStartList("/FAIL");

    if (strncmp(cname,"MATERIAL",8) != 0 && strncmp(cname,"PROPERTY",8) != 0  && 
        strncmp(cname,"PART",4) != 0     && strncmp(cname,"TRANSFORM",9) != 0 && 
        strncmp(cname,"SUBSET",6) != 0   && strncmp(cname,"FAILURE",7) != 0    ) GlobalModelSDISelectionStartList(cname);
//
}
