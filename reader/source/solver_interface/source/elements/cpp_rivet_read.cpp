...
// Get Submodel Id
        ...
        SUBID_RIVET[i] = includeId;
// Get Elem Id
        IXRI[*NIXRI * i + *NIXRI - 1] = elems->GetId()  ;
// Get Part Id
        IPARTRI[i] = elems->GetOwnerId();
// Get Elem Connectivity IXRI
        elems->GetNodeIds(aNodeId);
        for(size_t j=0; j < 2 ; ++j) IXRI[*NIXRI * i + j + 1] = aNodeId[j];
// next     
        i++;
        aNodeId.resize(0);
    }
