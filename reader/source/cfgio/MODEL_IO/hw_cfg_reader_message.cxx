...
HWCFGReaderMessage::HWCFGReaderMessage(unsigned int id, int type,
...
HWCFGReaderMessage::HWCFGReaderMessage()
    : p_id(0), p_type(0),
...
void HWCFGReaderMessage::CleanEnd(std::string& text)
{ 
...
HWCFGReaderMessageList & HWCFGReaderMessageList::operator =(const HWCFGReaderMessageList &other)
{
    std::vector<HWCFGReaderMessage>::operator=(other);
...
HWCFGReaderMessageList::~HWCFGReaderMessageList()
{
    if(p_owningMessagePool && p_pMessagePool) delete p_pMessagePool;
}

void HWCFGReaderMessageList::SetMessagePool(const HWCFGReaderMessagePool *pMessagePool,
                                            bool owningMessagePool)
{
    if(p_owningMessagePool && p_pMessagePool) delete p_pMessagePool;
...
const HWCFGReaderMessage& HWCFGReaderMessageList::Add(unsigned int id, int type,
                            const std::string &block, const std::string &line,
                            const std::string &filename, unsigned int linenumber,
                            ...)
{
    ...
    const HWCFGReaderMessage& message = Add(id, type, args,
    ...
    const std::string &filename, unsigned int linenumber)
{
    ...
    HWCFGReaderMessage message(id, type, block, line, filename, linenumber,
...
const HWCFGReaderMessage& HWCFGReaderMessageList::Add(
    const char *format, va_list args, int defaulttype,
    const std::string &block, const std::string &line,
    const std::string &filename, unsigned int linenumber)
{
    bool found = false;
    unsigned int id = GetDefaultId(defaulttype);
    ...
    else if(p_pMessagePool)
    {
        ...
        {
            unsigned int itId = it->first; // to be cleaned up: Add an iterator to HWCFGReaderMessagePool
    ...
    if(p_pMessagePool)
    {
        title = p_pMessagePool->GetTitle(id);
        solution = p_pMessagePool->GetSolution(id);
    }
...
void HWCFGReaderMessageList::Add(const std::map<int, std::vector<std::string>> &messages)
{
    for(auto it=messages.begin();it!=messages.end();it++)
    {
        for(auto it2=it->second.begin(); it2 != it->second.end(); ++it2)
        {
            HWCFGReaderMessage message(0, it->first, "", 0, *it2);
            push_back(message);
        }
    }
}
