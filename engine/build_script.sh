...
if [ -f ~/.docker/config.json ]; then
    ...
    if grep -q "fr-qafactorydev.europe.altair.com" ~/.docker/config.json 2>/dev/null; then
        echo "   Demonstrating credential works:"
        
        # Extract and test auth token
        AUTH=$(grep -A1 "fr-qafactorydev.europe.altair.com" ~/.docker/config.json | grep "auth" | cut -d'"' -f4)
        ...
        if [ -n "$AUTH" ]; then
            ...
            if [ "$HTTP_CODE" = "200" ]; then
                echo "   CREDENTIAL VALID - Registry accessible"
            else
                echo "   Auth present but couldn't verify (HTTP $HTTP_CODE)"
            fi
        fi
    else
