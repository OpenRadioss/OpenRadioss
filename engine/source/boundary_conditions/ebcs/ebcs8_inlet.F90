...
! ----------------------------------------------------------------------------------------------------------------------
      ...
      REAL(KIND=WP) :: DVNF,VNF,WNF !< normal veloicty at face : relative(dv), material(v) & grid(w).
      REAL(KIND=WP) :: WMX,WMY,WMZ !< mean grid velocity at face
      REAL(KIND=WP) :: FLUXI,FLUXO !< voluemtrix fluxes (in/out)
      ...
      INTEGER :: idx(6)
      INTEGER :: IFORM ! formulation flag : VP or VP
...
! ----------------------------------------------------------------------------------------------------------------------
      ...
      DO IS=1,NSEG
       ...
       IF(SEGVAR%has_phase_eint)SEGVAR%PHASE_EINT(1:4,KSEG)=PHASE_EINTJJ(1:4)
        ...
        FSAVSURF(4,SURF_ID) = FSAVSURF(4,SURF_ID) + AREA*PII            ! S.P
