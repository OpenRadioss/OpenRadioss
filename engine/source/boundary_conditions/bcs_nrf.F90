...
! ----------------------------------------------------------------------------------------------------------------------
          ...
          integer, intent(in) :: mode !< 0: compute forces, 1: compute stiffness after mpi communication
...
! ----------------------------------------------------------------------------------------------------------------------
                ...
                if(n2d == 0)then
                  ...
                  DO INOD = 1,MAXNOD
                    ...
                    if(iparit==0) then
                      ...
                      iad = bcs%cfl_nrf%iadsky(inod,jj+my_offset)                      
                      bcs%cfl_nrf%fsky(1,iad) = FACAREArCp*NX
                      bcs%cfl_nrf%fsky(2,iad) = FACAREArCp*NY
                      bcs%cfl_nrf%fsky(3,iad) = FACAREArCp*NZ    
                    endif
  ...
  !$omp end critical                  
                  ...
                  if(iparit==0) then
                    ...
                    iad = bcs%cfl_nrf%iadsky(1,jj+my_offset)
                    bcs%cfl_nrf%fsky(1,iad) = 0.
                    bcs%cfl_nrf%fsky(2,iad) = HAREArCp*NY
                    bcs%cfl_nrf%fsky(3,iad) = HAREArCp*NZ
                    iad = bcs%cfl_nrf%iadsky(2,jj+my_offset)                  
                    bcs%cfl_nrf%fsky(1,iad) = 0.
                    bcs%cfl_nrf%fsky(2,iad) = HAREArCp*NY
                    bcs%cfl_nrf%fsky(3,iad) = HAREArCp*NZ
                  endif
