...
! ----------------------------------------------------------------------------------------------------------------------
          ...
          !< Number of integer interface properties
          ...
          integer, dimension(mds_nmat), intent(in) :: mds_ndepsvar
          !< Number of dependent variables per material
...
5000        format(/ &
          ...
          if (h3d_data%n_scal_skid > 0 .and. nintskidold == 0) then
            ...
            if (nintstamp /= 0) then
              call my_alloc(pskids, ninterskid, numnodg, "pskids")
              pskids(1:ninterskid, 1:numnodg) = ZERO
            else
              call my_alloc(pskids, ninterskid, numnod, "pskids")
              pskids(1:ninterskid, 1:numnod) = ZERO
            endif
          elseif (h3d_data%n_scal_skid == 0 .and. nintskidold == 0) then
            call my_alloc(pskids, 0, 0, "pskids")
          endif

          ! Output by interface: 2 - Frictional energy allocations
          if (h3d_data%n_scal_cse_fric > 0 .and. output%data%s_efric == 0) then
            call my_alloc(output%data%efricg, numnod, "output%data%efricg")
            output%data%efricg(1:numnod) = ZERO
            if (nintstamp /= 0) then
              call my_alloc(output%data%efricg_stamp, numnodg, "output%data%efricg_stamp")
              output%data%efricg_stamp(1:numnodg) = ZERO
            else
              call my_alloc(output%data%efricg_stamp, 0, "output%data%efricg_stamp")
            endif
          elseif (output%data%s_efric == 0) then
            call my_alloc(output%data%efricg, 0, "output%data%efricg")
            call my_alloc(output%data%efricg_stamp, 0, "output%data%efricg_stamp")
          endif
          ...
          if (h3d_data%n_scal_cse_fricint > 0 .and. output%data%s_efricint == 0) then
            ...
            if (output%data%ninefric_stamp == ninefricg) then
              call my_alloc(output%data%efric_stamp, ninefricg, numnodg, "output%data%efric_stamp")
            ...
            elseif (output%data%ninefric_stamp == 0) then
              ...
              call my_alloc(output%data%efric, output%data%ninefric, numnod, "output%data%efric")
              ...
              call my_alloc(output%data%efric_stamp, output%data%ninefric_stamp, numnodg, "output%data%efric_stamp")
