...
/*=================================================================*/
{
                                                 ...
                                                 int *id, int *mode)
        {
                ...
                for (i = 0; i < *sizeMdsUvarName; i++)
                        cmdsuvar[i] = mdsUvarName[i];
                for (i = 0; i < *sizeMdsUvarName; i++)
                        if (isblank(cmdsuvar[i]))
                                cptBlank = cptBlank + 1;
                if (cptBlank == *sizeMdsUvarName)
                        isBlankVar = true;
                ...
                try
                {
                        ...
                        dt_id++;
#ifdef _WIN64
                        strcpy_s(edata_type, 100, RES_STRING);
#else
        ...
        void _FCALL C_H3D_CREATE_SHELL_SCALAR_DATATYPE(int *cpt_data, char *name1, int *size1, int *info1, int *info2, char *name2, int *size2,
                                                       ...
                                                       char *comment, int *s_comment, char *mdsUvarName, int *sizeMdsUvarName, int *id, int *mode)
        {
                c_h3d_create_shell_scalar_datatype_(cpt_data, name1, size1, info1, info2, name2, size2, layer, ipt, ply, nuvar, gauss, idMds, idMdsVar, idMatMds, comment, s_comment,
                                                    mdsUvarName, sizeMdsUvarName, id, mode);
        }
