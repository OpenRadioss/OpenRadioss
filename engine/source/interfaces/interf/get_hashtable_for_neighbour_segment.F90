          ...
          do i=1,nrtm
            do j=1,4
              ...
              if(segment_id/=0) then            
                ...
                address = shoot_struct%neighbour(nin)%seg_index(ijk) + tmp_address(ijk)
                shoot_struct%neighbour(nin)%mvoisin_index(address) = 4*(i-1)+j
                write(*,'(2i10,a,6i10)') my_ispmd,my_ncycle," INIT ",segment_id,nin,address,4*(i-1)+j,i,j
                tmp_address(ijk) = tmp_address(ijk) + 1
              endif
            enddo
          enddo
          ! ------------
