...
!                                                   local variables
! ----------------------------------------------------------------------------------------------------------------------
      integer :: m3, m2, m1, i, n, nindex, ifq, j, k, jj,index(nsn),ipri
      real(kind=WP) :: tfxtn,tfxt, dp, dv, da, dvt,fndfn, ftdft, fric,           &
                      fric2,fcoe,msw,fac,alpha,alphi,de,tfxt2, tfxtn2, wewe2,    &
                      ...
                      dsm_x1,dsm_x2,stf_vi,damp,stf_kt,fact,stf_av,              &
...
!                                                   Body
! ----------------------------------------------------------------------------------------------------------------------
! call it when a is the internal force, a,v in kinetic part are just after caling of acceleration
      ncont=0
      CALL MY_BARRIER
      stf_maxi=zero
      stf_av=zero
...
!
           ...
           pen_old(i) = pene(i)
           stf_kt = stif_kt(j)*(one+damp2)
           stifn(n)=stifn(n)+stf_kt
           stf_maxi = max(stf_maxi,stf_kt)
