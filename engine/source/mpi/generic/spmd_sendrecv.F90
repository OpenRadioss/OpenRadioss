!Copyright>        OpenRadioss
!Copyright>        Copyright (C) 2026 Siemens
!Copyright>
!Copyright>        This program is free software: you can redistribute it and/or modify
!Copyright>        it under the terms of the GNU Affero General Public License as published by
!Copyright>        the Free Software Foundation, either version 3 of the License, or
!Copyright>        (at your option) any later version.
!Copyright>
!Copyright>        This program is distributed in the hope that it will be useful,
!Copyright>        but WITHOUT ANY WARRANTY; without even the implied warranty of
!Copyright>        MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
!Copyright>        GNU Affero General Public License for more details.
!Copyright>
!Copyright>        You should have received a copy of the GNU Affero General Public License
!Copyright>        along with this program.  If not, see <https://www.gnu.org/licenses/>.
!Copyright>
!Copyright>
!Copyright>        Commercial Alternative: Simcenter Radioss Software
!Copyright>
!Copyright>        As an alternative to this open-source version, Siemens also offers Simcenter(TM) Radioss(R)
!Copyright>        software under a commercial license.  Contact Siemens to discuss further if the
!Copyright>        commercial version may interest you: 
!Copyright>        https://www.siemens.com/en-us/products/simcenter/mechanical-simulation/radioss/.
      module spmd_sendrecv_mod
        use spmd_comm_world_mod, only: SPMD_COMM_WORLD
        implicit none

        integer, parameter, public :: TAG_SENDRECV = -24

        !> \\brief Interface for spmd_sendrecv, a wrapper for MPI_SENDRECV
        interface spmd_sendrecv
          module procedure spmd_sendrecv_reals
          module procedure spmd_sendrecv_ints
          module procedure spmd_sendrecv_doubles
          module procedure spmd_sendrecv_reals2d
          module procedure spmd_sendrecv_ints2d
          module procedure spmd_sendrecv_doubles2d
          module procedure spmd_sendrecv_real
          module procedure spmd_sendrecv_int
          module procedure spmd_sendrecv_double
        end interface spmd_sendrecv

      contains

! ======================================================================================================================
!>  \\brief Sendrecv of real       array
        subroutine spmd_sendrecv_reals(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          real, dimension(:), intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          real, dimension(:), intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if (present(status)) then
            call MPI_Sendrecv(sendbuf, sendcount, MPI_REAL, dest, sendtag, recvbuf, recvcount, MPI_REAL, source, recvtag,&
            & used_comm, status, ierr)
          else
            call MPI_Sendrecv(sendbuf, sendcount, MPI_REAL, dest, sendtag, recvbuf, recvcount, MPI_REAL, source, recvtag,&
            & used_comm, local_status, ierr)
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_reals

! ======================================================================================================================
!>  \\brief Sendrecv of integer       array
        subroutine spmd_sendrecv_ints(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          integer, dimension(:), intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          integer, dimension(:), intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if (present(status)) then
            call MPI_Sendrecv(sendbuf, sendcount, MPI_INTEGER, dest, sendtag, recvbuf, recvcount, MPI_INTEGER, source, recvtag,&
            & used_comm, status, ierr)
          else
            call MPI_Sendrecv(sendbuf, sendcount, MPI_INTEGER, dest, sendtag, recvbuf, recvcount, MPI_INTEGER, source, recvtag,&
            & used_comm, local_status, ierr)
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_ints

! ======================================================================================================================
!>  \\brief Sendrecv of double precision       array
        subroutine spmd_sendrecv_doubles(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          double precision, dimension(:), intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          double precision, dimension(:), intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if (present(status)) then
            call MPI_Sendrecv(sendbuf, sendcount, MPI_DOUBLE_PRECISION, dest, sendtag, recvbuf, recvcount, MPI_DOUBLE_PRECISION,&
            & source, recvtag, used_comm, status, ierr)
          else
            call MPI_Sendrecv(sendbuf, sendcount, MPI_DOUBLE_PRECISION, dest, sendtag, recvbuf, recvcount, MPI_DOUBLE_PRECISION,&
            & source, recvtag, used_comm, local_status, ierr)
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_doubles

! ======================================================================================================================
!>  \\brief Sendrecv of real       array
        subroutine spmd_sendrecv_reals2d(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          real, dimension(:,:), intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          real, dimension(:,:), intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if (present(status)) then
            call MPI_Sendrecv(sendbuf, sendcount, MPI_REAL, dest, sendtag, recvbuf, recvcount, MPI_REAL, source, recvtag,&
            & used_comm, status, ierr)
          else
            call MPI_Sendrecv(sendbuf, sendcount, MPI_REAL, dest, sendtag, recvbuf, recvcount, MPI_REAL, source, recvtag,&
            & used_comm, local_status, ierr)
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_reals2d

! ======================================================================================================================
!>  \\brief Sendrecv of integer       array
        subroutine spmd_sendrecv_ints2d(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          integer, dimension(:,:), intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          integer, dimension(:,:), intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if (present(status)) then
            call MPI_Sendrecv(sendbuf, sendcount, MPI_INTEGER, dest, sendtag, recvbuf, recvcount, MPI_INTEGER, source, recvtag,&
            & used_comm, status, ierr)
          else
            call MPI_Sendrecv(sendbuf, sendcount, MPI_INTEGER, dest, sendtag, recvbuf, recvcount, MPI_INTEGER, source, recvtag,&
            & used_comm, local_status, ierr)
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_ints2d

! ======================================================================================================================
!>  \\brief Sendrecv of double precision       array
        subroutine spmd_sendrecv_doubles2d(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm,&
        & tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          double precision, dimension(:,:), intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          double precision, dimension(:,:), intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if (present(status)) then
            call MPI_Sendrecv(sendbuf, sendcount, MPI_DOUBLE_PRECISION, dest, sendtag, recvbuf, recvcount, MPI_DOUBLE_PRECISION,&
            & source, recvtag, used_comm, status, ierr)
          else
            call MPI_Sendrecv(sendbuf, sendcount, MPI_DOUBLE_PRECISION, dest, sendtag, recvbuf, recvcount, MPI_DOUBLE_PRECISION,&
            & source, recvtag, used_comm, local_status, ierr)
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_doubles2d

! ======================================================================================================================
!>  \\brief Sendrecv of real       scalar
        subroutine spmd_sendrecv_real(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          real,  intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          real,  intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if ((sendcount .ne. 1) .or. (recvcount .ne. 1)) then
            ierr = -1
          else
            if (present(status)) then
              call MPI_Sendrecv(sendbuf, sendcount, MPI_REAL, dest, sendtag, recvbuf, recvcount, MPI_REAL, source, recvtag,&
              & used_comm, status, ierr)
            else
              call MPI_Sendrecv(sendbuf, sendcount, MPI_REAL, dest, sendtag, recvbuf, recvcount, MPI_REAL, source, recvtag,&
              & used_comm, local_status, ierr)
            end if
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_real

! ======================================================================================================================
!>  \\brief Sendrecv of integer       scalar
        subroutine spmd_sendrecv_int(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          integer,  intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          integer,  intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if ((sendcount .ne. 1) .or. (recvcount .ne. 1)) then
            ierr = -1
          else
            if (present(status)) then
              call MPI_Sendrecv(sendbuf, sendcount, MPI_INTEGER, dest, sendtag, recvbuf, recvcount, MPI_INTEGER, source, recvtag,&
              & used_comm, status, ierr)
            else
              call MPI_Sendrecv(sendbuf, sendcount, MPI_INTEGER, dest, sendtag, recvbuf, recvcount, MPI_INTEGER, source, recvtag,&
              & used_comm, local_status, ierr)
            end if
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_int

! ======================================================================================================================
!>  \\brief Sendrecv of double precision       scalar
        subroutine spmd_sendrecv_double(sendbuf, sendcount, dest, sendtag, recvbuf, recvcount, source, recvtag, status, comm, tag)
          use spmd_error_mod, only: spmd_in, spmd_out
          implicit none
#include "spmd.inc"
          integer, intent(in) :: sendcount, dest, sendtag
          double precision,  intent(in) :: sendbuf
          integer, intent(in) :: recvcount, source, recvtag
          double precision,  intent(inout) :: recvbuf
          integer, intent(inout), optional :: status(MPI_STATUS_SIZE)
          integer, intent(in), optional :: comm
          integer, intent(in), optional :: tag
          integer :: ierr, used_comm
          integer :: tag_local
          integer :: local_status(MPI_STATUS_SIZE)

          if (present(tag)) then
            tag_local = tag
          else
            tag_local = TAG_SENDRECV
          end if

#ifdef MPI
          call spmd_in(tag_local, "MPI_Sendrecv")
          if (present(comm)) then
            used_comm = comm
          else
            used_comm = SPMD_COMM_WORLD
          end if

          if ((sendcount .ne. 1) .or. (recvcount .ne. 1)) then
            ierr = -1
          else
            if (present(status)) then
              call MPI_Sendrecv(sendbuf, sendcount, MPI_DOUBLE_PRECISION, dest, sendtag, recvbuf, recvcount, MPI_DOUBLE_PRECISION,&
              & source, recvtag, used_comm, status, ierr)
            else
              call MPI_Sendrecv(sendbuf, sendcount, MPI_DOUBLE_PRECISION, dest, sendtag, recvbuf, recvcount, MPI_DOUBLE_PRECISION,&
              & source, recvtag, used_comm, local_status, ierr)
            end if
          end if

          call spmd_out(tag_local, ierr)
#else
          recvbuf = sendbuf
          if (present(status)) status = 0
#endif
        end subroutine spmd_sendrecv_double

      end module spmd_sendrecv_mod
