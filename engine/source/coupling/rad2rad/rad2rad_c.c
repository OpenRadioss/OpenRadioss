...
#endif
#if CPP_mach==CPP_sp2 || CPP_mach==CPP_sgi6_spmd || CPP_mach==CPP_hp9_spmd || CPP_mach==CPP_hp10_spmd || CPP_mach==CPP_pwr4_spmd
#include <sys/mman.h>
#endif
...
static char fifo_1[NAMESIZE], fifo_2[NAMESIZE], semaphore_int[NAMESIZE];	/* private fifos */
...
/*---------------------------------------------------------------------*/
extern int readr (int pipe, char* buf, int nbytes);
extern int writer(int pipe, char* buf, int nbytes);
...
#if CPP_mach == CPP_w95 || CPP_mach == CPP_win64_spmd || CPP_mach == CPP_p4win64_spmd || CPP_mach == CPP_wnt || CPP_mach == CPP_wmr || CPP_mach == CPP_p4win64 || CPP_mach == CPP_p4win32 
      sock = *sd;
	  if (recv(sock,messtop,3*sizeof(char), 0) == -1){
                    perror("recv top fifo");
                    exit(1);}  	  
	  fidr = CreateFile(&fifo_1,GENERIC_READ,0,NULL,OPEN_EXISTING,0,NULL);
	  if (fidr == INVALID_HANDLE_VALUE) 
         fatal("Can't open fifo 1");
	  fidw = CreateFile(&fifo_2,GENERIC_WRITE,0,NULL,OPEN_EXISTING,0,NULL);
	  if (fidw == INVALID_HANDLE_VALUE) 
         fatal("Can't open fifo 2");
      if (send(sock,buftop,3*sizeof(char), 0) == -1){
...
#endif
         if((com = (my_real_c *) malloc(sizeof(struct region))) == NULL) syserr_fatal("Malloc");
...
int i;
    ...
    for (i = 0; i < *nthr; i++)
        {
#if CPP_mach == CPP_w95 || CPP_mach == CPP_win64_spmd || CPP_mach == CPP_p4win64_spmd || CPP_mach == CPP_wnt || CPP_mach == CPP_wmr || CPP_mach == CPP_p4win64 || CPP_mach == CPP_p4win32
	 if (!ReleaseSemaphore(sem_int,1,NULL) ) {printf("release semaphore\n", GetLastError());}
#elif 1		
    ...
    {                
        ...
        for (next = 0; next < rest; next++)   {                  
	       ...
	       if (flg_sphinout==1)
		    ...
		    weight = 0;}
                 else if (com->iactv[i] == -1)
	           {/* deactivation of particle */
		    x[nn+j] = com->fx_buf[k+j];
	            if (*iresp==1) xdp[nn+j] = com->fx_buf[k+j];
	            com->fx_buf[k+j] = 0;
		    v[nn+j] = 0;
...
int *sd;
{
...
#if (CPP_mach == CPP_win64_spmd || CPP_mach == CPP_p4win64_spmd || CPP_mach == CPP_p4win64  || CPP_mach == CPP_p4win32 || CPP_mach == CPP_wnt && CPP_rel == 10)
  SOCKET sock;
  SOCKADDR_IN server;
#else
 ...
 /********************Iteration on ports to find a free one***************/
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_LINGER, &so_linger, sizeof(so_linger))==-1) {perror("setsockopt linger");exit(1);}     
...
void _FCALL CONNECTION_SOCK_INIT_C(sd)
int *sd;
{connection_sock_init_c(sd);}

void connection_sock_init_c_(sd)
int *sd;
{connection_sock_init_c(sd);}

void connection_sock_init_c__(sd)
int *sd;
{connection_sock_init_c(sd);}

void send_sock_init_c(iroot,len,ispmd,sd,maxproc,imach)
int *iroot,*len,*ispmd,*sd,*maxproc,*imach;
{
  ...
  char PUF[512];
	...
	root[i] = (char) iroot[i];
     ...
     if (send(sock,root,70*sizeof(char), 0) == -1){
     perror("send name");
     exit(1);}
     
     if (send(sock,&nb,sizeof(nb), 0) == -1){
     perror("send nb proc");
     exit(1);}
     
     if (send(sock,&vers,sizeof(vers), 0) == -1){
     perror("send version");
     exit(1);}
     
     if (send(sock,&imachine,sizeof(vers), 0) == -1){
     perror("send imach");
     exit(1);}                      
    
    if (recv(sock,&flag_error,sizeof(flag_error), 0) <= 0){
    perror("recv rad2rad message");
    exit(1);}
    if (flag_error == 1) {printf("\n error message sent by rad2rad\n");exit(1);}    

    if (nb != 0){
    if (recv(sock,nom,sizeof(nom), 0) <= 0){
    perror("recv top");
    exit(1);}}                          
}
...
int *sd;
{
  ...
  char nom[512];                    
     ...
     if (recv(sock,nom,3*sizeof(char), 0) == -1)
        {perror("recv norm");
         exit(1);}     
}
...
#endif
     sock=*sd;
     if (recv(sock,ibuf,*len*sizeof(int), 0) == -1){
...
#endif
     sock=*sd;
     if (send(sock,ibuf,*len*sizeof(int), 0) == -1){
     perror("send sock ibuf ");
     exit(1);}     
}
...
#endif
     sock=*sd;
     if (send(sock,rbuf,*len*sizeof(my_real_c), 0) == -1){
...
#endif
     sock=*sd;
     if (recv(sock,rbuf,*len*sizeof(my_real_c), 0) == -1){
     perror("send sock rbuf ");
     exit(1);}     
}
...
#endif
     sock=*sd;
     if (send(sock,mess,*len*sizeof(char), 0) == -1){
...
#endif
     sock=*sd;
     if (recv(sock,mess,*len*sizeof(char), 0) == -1){
     perror("send sock mess ");
     exit(1);}     
}
...
int i,*shmloc,new_tot_size,old_tot_size;
  ...
  newsize = max(newsize,old_tot_size+150);
...
void exch_tagel_c(ntagel,tagel,flag)
...
#define BUFSIZE	 	3*PIPE_BUF
#define buftop          "top"
void openfifo_c(iroot,len,fdw,fdr,sd)
int *iroot, *len, *fdw, *fdr, *sd;
{
int i, naps, sock;
	printf(" RAD2RAD NOT AVAILABLE FOR THIS VERSION \n");
	arret_c(2);
}


void _FCALL OPENFIFO_C(iroot,len,fdw,fdr,sd)
int *iroot, *len, *fdw, *fdr, *sd;
{
	openfifo_c(iroot,len,fdw,fdr,sd);
}
void openfifo_c_(iroot,len,fdw,fdr,sd)
int *iroot, *len, *fdw, *fdr, *sd;
{
	openfifo_c(iroot,len,fdw,fdr,sd);
}
void openfifo_c__(iroot,len,fdw,fdr,sd)
int *iroot, *len, *fdw, *fdr, *sd;
{openfifo_c(iroot,len,fdw,fdr,sd);}


void init_link_c(igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink)
int *igd, *nng, *nodbuf, *itab,*addcnel,*cnel,*ixc,*ofc,*info,*typ,*cdt,*cdr,*print,*rddl,*nlink;
my_real_c *x;
{
int i,j,t,nn, lbuf, lbuf1, init_buf[6],lbufa,lbufb,lbufel,dimno,k,capt,g,m,w,nbel,id,ref,lus;
...
void _FCALL  INIT_LINK_C (igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink)
int *igd, *nng, *nodbuf, *itab,*addcnel,*cnel,*ixc,*ofc,*info,*typ,*cdt,*cdr,*print,*rddl,*nlink;
my_real_c *x;
{
	init_link_c(igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink);
}

void _FCALL init_link_c_(igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink)
int *igd, *nng, *nodbuf, *itab,*addcnel,*cnel,*ixc,*ofc,*info,*typ,*cdt,*cdr,*print,*rddl,*nlink;
my_real_c *x;
{
	init_link_c(igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink);
...
void _FCALL init_link_c__(igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink)
int *igd, *nng, *nodbuf, *itab,*addcnel,*cnel,*ixc,*ofc,*info,*typ,*cdt,*cdr,*print,*rddl,*nlink;
my_real_c *x;
{init_link_c(igd, nng, itab, nodbuf, x,addcnel,cnel,ixc,ofc,info,typ,cdt,cdr,print,rddl,nlink);}
...
/*void init_link_spmd_c(igd, nng, ibuf, rbuf)
int *igd, *nng, *ibuf;
my_real_c *rbuf;
{
int lbuf, lbuf1, init_buf[2];
	init_buf[0] = *igd;
	init_buf[1] = *nng;
	writer(fidw, (void *) init_buf, 2*sizeof(int));	
	lbuf = *nng*sizeof(my_real_c);
	lbuf1  = *nng*sizeof(int);
	writer(fidw, (void *) ibuf, lbuf1);	
	writer(fidw, (void *) rbuf, 3*lbuf);	
 Cmw51h3		readr(fidr, (void *)activ, sizeof(int));}*/

void init_link_spmd_c(igd,nng,dbnod,nbproc,ibuf,dbibuf,dbnbuf,ddbuf,rbuf,dim,ibufnb,ibufcnel,nbel,dimel,ibufel,ibufelnbnod,ibufelnod,dimb,ibufcneldb,ibufnbeldb,typ,bcs,print,rddl,nl,nlnk,iex)
int *igd, *nng, *nbproc, *ibuf, *dbibuf,*dbnbuf,*ddbuf,*dbnod,*dim,*ibufnb,*ibufcnel,*nbel,*dimel,*ibufel,*ibufelnbnod,*ibufelnod,*dimb,*ibufcneldb,*ibufnbeldb,*typ,*bcs,*print,*rddl,*nl,*nlnk,*iex;
my_real_c *rbuf;
{
int lbuf, lbuf1,lbufb,lbufp,init_buf[8],lbufa,lbufel,lbufdimel,lbufc;
	printf(" RAD2RAD NOT AVAILABLE FOR THIS VERSION \n");
	arret_c(2);
}
...
void send_mass_spmd_c(idp, nng, buf1, buf2)
int *idp, *nng;
my_real_c *buf1, *buf2;
{
int lbuf, flag;
...
void _FCALL SEND_MASS_SPMD_C(idp, nng, buf1, buf2)
int *idp, *nng;
my_real_c *buf1, *buf2;
{
	send_mass_spmd_c(idp, nng, buf1, buf2);
}
void send_mass_spmd_c_(idp, nng, buf1, buf2)
int *idp, *nng;
my_real_c *buf1, *buf2;
{
	send_mass_spmd_c(idp, nng, buf1, buf2);
}
void send_mass_spmd_c__(idp, nng, buf1, buf2)
int *idp, *nng;
my_real_c *buf1, *buf2;
{send_mass_spmd_c(idp, nng, buf1, buf2);}
...
void send_data_c(idp, nng, nodbuf, fx, fr, stx, str, vx,  vr,  ms,  in,  dx)
int *idp, *nng, *nodbuf;
my_real_c *fx, *fr, *stx, *str, *vx, *vr, *ms, *in, *dx;
{
int buflen, lbuf, rest, next;
int i, j, k, nn, nm;
...
/******************************************************************/
void _FCALL SEND_DATA_C(idp, nng, nodbuf, fx, fr, stx, str,vx,  vr,  ms,  in,  dx)
int *idp, *nng, *nodbuf;
my_real_c *fx, *fr, *stx, *str, *vx, *vr, *ms, *in, *dx;
{
	send_data_c(idp, nng, nodbuf, fx, fr, stx, str, vx,  vr,  ms,  in,  dx);
}
void send_data_c_(idp, nng, nodbuf,fx, fr, stx, str, vx,  vr,  ms,  in,     dx)
int *idp, *nng, *nodbuf;
my_real_c *fx, *fr, *stx, *str, *vx, *vr, *ms, *in, *dx;
{
	send_data_c(idp, nng, nodbuf, fx, fr, stx, str, vx,  vr,  ms,  in,  dx);
}
void send_data_c__(idp, nng, nodbuf,fx, fr, stx, str, vx,  vr,  ms,  in,  dx)
int *idp, *nng, *nodbuf;
my_real_c *fx, *fr, *stx, *str, *vx, *vr, *ms, *in, *dx;
{send_data_c(idp, nng, nodbuf, fx, fr, stx, str, vx,  vr,  ms,  in,  dx);}
/******************************************************************/
...
/* el51g1+++ */
void send_data_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4, *bufr5, *bufr6, *bufr7, *bufr8, *bufr9;
{
int buflen, lbuf, rest, next;
	printf(" RAD2RAD NOT AVAILABLE FOR THIS VERSION \n");
	arret_c(2);
...
/******************************************************************/
void _FCALL SEND_DATA_SPMD_C(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4, *bufr5, *bufr6, *bufr7, *bufr8, *bufr9;
{
	send_data_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9);
}
void send_data_spmd_c_(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4, *bufr5, *bufr6, *bufr7, *bufr8, *bufr9;
{
	send_data_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9);
}
void send_data_spmd_c__(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4, *bufr5, *bufr6, *bufr7, *bufr8, *bufr9;
{
        send_data_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4, bufr5, bufr6, bufr7, bufr8, bufr9);}
...
void get_stiff_c(idp, nng, nodbuf, ms, ir, stx, str)
int *idp, *nng, *nodbuf;
my_real_c *ms, *ir, *stx, *str;
{
int buflen, lbuf, rest, next;
...
/******************************************************************/
void _FCALL GET_STIFF_C(idp, nng, nodbuf, ms, ir, stx, str)
int    *idp, *nng, *nodbuf;
my_real_c *ms, *ir, *stx, *str;
{   get_stiff_c(idp, nng, nodbuf, ms, ir, stx, str);  }

void get_stiff_c_(idp, nng, nodbuf, ms, ir, stx, str)
int    *idp, *nng, *nodbuf;
my_real_c *ms, *ir, *stx, *str;
{   get_stiff_c(idp, nng, nodbuf, ms, ir, stx, str);  }

void get_stiff_c__(idp, nng, nodbuf, ms, ir, stx, str)
int    *idp, *nng, *nodbuf;
my_real_c *ms, *ir, *stx, *str;
{   get_stiff_c(idp, nng, nodbuf, ms, ir, stx, str);  }
/******************************************************************/
...
void get_stiff_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{
int buflen, lbuf, rest, next;
...
/******************************************************************/
void _FCALL GET_STIFF_SPMD_C(idp, nng, bufr1, bufr2, bufr3, bufr4)
int    *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{   get_stiff_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4);  }

void get_stiff_spmd_c_(idp, nng, bufr1, bufr2, bufr3, bufr4)
int    *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{   get_stiff_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4);  }

void get_stiff_spmd_c__(idp, nng, bufr1, bufr2, bufr3, bufr4)
int    *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{   get_stiff_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4);  }
/******************************************************************/
...
void get_force_c(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in)
int *idp, *nng, *nodbuf;
my_real_c *wf, *wm, *wf2, *wm2, *v, *vr, *fx, *fr, *ms, *in;
{
int buflen, lbuf, rest, next;
int i, j, k, nn, nm;
my_real_c df, dm;
...
/******************************************************************/
void _FCALL GET_FORCE_C(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in)
int *idp, *nng, *nodbuf;
my_real_c *wf, *wm, *wf2, *wm2, *v, *vr, *fx, *fr, *ms, *in;
{	get_force_c(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in);}
void get_force_c_(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in)
int *idp, *nng, *nodbuf;
my_real_c *wf, *wm, *wf2, *wm2, *v, *vr, *fx, *fr, *ms, *in;
{	get_force_c(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in);}
void get_force_c__(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in)
int *idp, *nng, *nodbuf;
my_real_c *wf, *wm, *wf2, *wm2, *v, *vr, *fx, *fr, *ms, *in;
{	get_force_c(idp, nng, nodbuf, wf, wm, wf2, wm2, v, vr, fx, fr, ms, in);}
/******************************************************************/

void get_force_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{
int buflen, lbuf, rest, next;
...
/******************************************************************/
void _FCALL GET_FORCE_SPMD_C(idp, nng, bufr1, bufr2, bufr3, bufr4)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{	get_force_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4);}
void get_force_spmd_c_(idp, nng, bufr1, bufr2, bufr3, bufr4)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{	get_force_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4);}
void get_force_spmd_c__(idp, nng, bufr1, bufr2, bufr3, bufr4)
int *idp, *nng;
my_real_c *bufr1, *bufr2, *bufr3, *bufr4;
{	get_force_spmd_c(idp, nng, bufr1, bufr2, bufr3, bufr4);}
...
{
  int num,i,nb,flags,compt,capt,port2;
  int code,spmd,vers;
  struct  hostent *hp;
	printf(" RAD2RAD NOT AVAILABLE FOR THIS VERSION \n");
	arret_c(2);
...
void _FCALL CONNECTION_SOCK_INIT_C(iroot,len,ispmd,sd,maxproc)
int *iroot,*len,*ispmd,*sd,*maxproc;
{connection_sock_init_c(iroot,len,ispmd,sd,maxproc);}

void connection_sock_init_c_(iroot,len,ispmd,sd,maxproc)
int *iroot,*len,*ispmd,*sd,*maxproc;
{connection_sock_init_c(iroot,len,ispmd,sd,maxproc);}

void connection_sock_init_c__(iroot,len,ispmd,sd,maxproc)
int *iroot,*len,*ispmd,*sd,*maxproc;
{connection_sock_init_c(iroot,len,ispmd,sd,maxproc);}
