...
//         if (!engine_user_initialize) err=err+1;
    ...
    sprintf(rname,"ENG_CUSER29");
    eng_cuser29=(void*)GetProcAddress(userlibhandler,rname);
    if(eng_cuser29) {
         dlib_array[3]=1;
    }

    sprintf(rname,"ENG_CUSER30");
    eng_cuser30=(void*)GetProcAddress(userlibhandler,rname);
    if(eng_cuser30) {
         dlib_array[4]=1;
    }

    sprintf(rname,"ENG_CUSER31");
    eng_cuser31=(void*)GetProcAddress(userlibhandler,rname);
    if(eng_cuser31) {
         dlib_array[5]=1;
    }  
...
void dyn_userlib_init_(char * libname, int *size, int * userlib_avail, int * userlib_ver ,int * iresp,int * userlib_altname, int * dlib_array){
    ...
    if(*userlib_altname==0){
       ...
       *size = *size + sz;
       ...
       libname[*size  ]='\0';
       if (*iresp ==0){
          strcpy(libname_c,"linux64.so");
       }else{
          strcpy(libname_c,"linux64_sp.so");
       }
       sz=strlen(libname_c);
       strcat(libname,libname_c);
       *size = *size + sz;
#endif
     }
     libn = (char * )malloc(sizeof(char)* *size+1);
...
/* Routine ENGINE_USER_INITIALIZE*/
     ...
     sprintf(rname,"cuser29_");
     eng_cuser29=dlsym(userlibhandle,rname);

     sprintf(rname,"cuser30_");
     eng_cuser30=dlsym(userlibhandle,rname);

     sprintf(rname,"cuser31_");
     eng_cuser31=dlsym(userlibhandle,rname);
...
// eng_userlib_get_law_user_var_2
// -------------------------------
...
// User Spring 29-30-31 properties
// --------------------------------- 
...
// User shell properties for 29-30-31 shell elements
// -------------------------------------------------
...
void _FCALL engine_userlib_cuser30_(
...
void _FCALL engine_userlib_cuser31_(
      int*NEL         ,int * NNOD       ,int*NUVAR   ,int*IPROP  ,int*IMAT  ,int*SOLID_ID,my_real *TIME  ,my_real *TIMESTEP,
      my_real *EINT   ,my_real *VOL    ,my_real *UVAR   ,my_real *FR_WAVE,my_real *OFF    ,my_real *RHO    ,my_real *SIG    ,
      my_real *XX     ,my_real *YY     ,my_real *ZZ     ,my_real *UX     ,my_real *UY     ,my_real *UZ     ,
      my_real *VX     ,my_real *VY     ,my_real *VZ     ,my_real *VRX    ,my_real *VRY    ,my_real *VRZ    ,
