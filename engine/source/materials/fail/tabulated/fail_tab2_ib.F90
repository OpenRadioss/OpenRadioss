...
!c-----------------------------------------------
          ...
          real(kind=WP), dimension(nel)     ,intent(inout)  :: uelr     ! integration point deactivation flag
          ...
          real(kind=WP), dimension(nel)     ,intent(in)     :: signzx   ! stress component zx
          ...
          real(kind=WP), dimension(nel)     ,intent(inout)  :: dmg_scale ! damage scale
...
!                                                  local variables
!c-----------------------------------------------
          integer :: i, j, nindx, failip, ireg, ndim, log_scale1, log_scale2
          ...
          real(kind=WP), dimension(nel) :: inst, dc, l0, triax, xi, epsf, epsl, &
            depsf, sizefac, ratefac, softexp, dlim, tempfac, tempfac2
...
!c===============================================================================================
          ...
          failip        = min(nint(fail%uparam(2)),npg) !> number of failed integration point prior to solid element deletion
