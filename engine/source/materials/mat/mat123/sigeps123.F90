...
! ----------------------------------------------------------------------------------------------------------------------          
      integer :: i,nfail,nkey,ipos1(1,1),ipos(nel,1),type,debug,ndx,n,nfunc
      integer :: indx(nel)
...
!!======================================================================
      ...
      !! orthotropic hook 3D
       ...
       !
       if(time == zero ) uvar(:,16) = (vol(:)** third) ! intial characteristic length 
       debug = 0
       if(time == zero) uvar(1:nel,16) = vol(1:nel)**third ! carateristic lenght at first step
             ...
             ! Kinking  check of failure 
              if( dmg(i,6) == one .and. dkink < one  ) then ! Kinking failure
                  ...
                  ! =============================================
                   ...
                   ! ===============================================================================
                   ! rotation of plane (a,b_psi) - with thetai (misalignement angle) ===>(am, bm)
                 ...
                 elseif(sigma_a < zero .and. dmg(i,3) == zero ) then
                  ...
                  ! =============================================
                    ...
                    !=======================================
                     ...
                     theta  = sign(one, tau_ab_psi)*(thetai + gamai)
                    !=====================================================================
                    ! step 2 : rotation of plane (a,b_psi) - with theta (misalignement angle) ===> 
                    ! ============================================================================
                     ! Precompute trigonometric terms
                     c = cos(theta)
                     s = sin(theta)
