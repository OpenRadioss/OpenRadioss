...
!----------------------------------------------------------------
          ...
          integer,       dimension(nel),       intent(in)     :: ngl       !< Emement global ID
...
! ----------------------------------------------------------------------------------------------------------------------
          integer :: i,j,nindxc,nindxuc,indxc(nel),indxuc(nel),itype,k,i1,i2,  &
            i3,ipos(nel,20),k1,ibulk,lcsrtmp,shdflg,ipru,nindxoff(nel,6)
          real(kind=WP) :: young,nu,shear,bulk,sigy0,vf,mu,eaau,ebbu,eccu,gabu,&
            ...
            ssef,a,bulkv,shearv,aaa,abb,acc,aab,abc,aca
          ...
          real(kind=WP), dimension(nel) :: vr,beta,eaa,ebb,ecc,gab,gbc,gca,    &
            ...
            sxx,syy,szz,sxy,syz,szx,seq,scale,sigb,dsigb,sigs,dsigs,sigw,dsigw,&
...
!
                ! Strong axis = direction A = e_x dans le repère matéria
                do j = 1, nindxuc
...
!
        ...
        contains
          ...
          subroutine enforce_plane_orientation(nplane, A, vperp, vother)
            ...
            ! Vérifier si A // n : si oui, on ne modifie pas la base
            cosNA = abs( n(1)*A(1) + n(2)*A(2) + n(3)*A(3) )
            if (cosNA > 1.0_WP - 1.0e-8_WP) then
              ! base déjà correcte, on renvoie des marqueurs "ne rien faire"
            ...
            if (nrm <= 1.0e-12_WP) then
              ! cas pathologique, mais à ce stade il ne devrait plus arriver
              vperp  = (/ 0.0_WP, 0.0_WP, 0.0_WP /)
              vother = (/ 0.0_WP, 0.0_WP, 0.0_WP /)
              return
            endif
