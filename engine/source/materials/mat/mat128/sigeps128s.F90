...
!---------------------------------------------------------------------
      ...
      if (fisokin > zero) then 
        sigbxx(1:nel) => sigb(1:nel,1)
        sigbyy(1:nel) => sigb(1:nel,2)
        sigbzz(1:nel) => sigb(1:nel,3)
        sigbxy(1:nel) => sigb(1:nel,4)
        sigbyz(1:nel) => sigb(1:nel,5)
        sigbzx(1:nel) => sigb(1:nel,6)
        ...
        if (mat_param%ntable == 0) then     ! analytical yield formulation
         ...
         h0(1:nel)   = qr1*cr1 + qr2*cr2 + qx1*cx1 + qx2*cx2
          ...
          if (ndim == 2) then
            ...
            call table_mat_vinterp(itable,nel,nel,vartmp,xvec2,yld0,h0)
          end if       
        end if
      endif
!---------------------------------------------------------------------
