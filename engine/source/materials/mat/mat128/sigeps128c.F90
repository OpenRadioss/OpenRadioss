...
! ----------------------------------------------------------------------------------------------------------------------
          ...
          real(kind=WP) :: dezz
          real(kind=WP) :: a,b,s12,s1,s2,s3
          real(kind=WP) :: a_1,a_2,a_3,p_1,p_2,p_3,pp1,pp2,pp3,dr,dr0
          real(kind=WP) :: fhk, fa01, fa02, fa03
          real(kind=WP) :: yld_i,ht
          real(kind=WP) ,dimension(nel)   :: normxx,normyy,normxy
          real(kind=WP) ,dimension(nel)   :: axx,ayy,axy,cxy
          real(kind=WP) ,dimension(nel)   :: jq,jq2
          real(kind=WP) ,dimension(nel)   :: s11,s22
          real(kind=WP) ,dimension(nel)   :: b_1,b_2,b_3,q12,q21
...
!
                ...
                dphi_dlam(i) = -dsig_dlam - h(i)*dpla_dlam(i) + dsigb_dlam
