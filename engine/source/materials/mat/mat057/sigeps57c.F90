...
!-------------------------------------------------------------------------------
        ...
        my_real, dimension(nel), intent(inout)         :: off      !< Element deletion status
        my_real, dimension(nel), intent(inout)         :: etse     !< Hourglass control coefficient
        ...
        my_real, intent(in)                            :: asrate   !< Strain rate filtering coefficient
        my_real, dimension(nel), intent(inout)         :: epsp     !< Equivalent strain rate (total or plastic)
        my_real, dimension(nel), intent(inout)         :: epsd     !< Strain rate used in output & equations
        integer, intent(in)                            :: inloc    !< Non-local thickness variation flag
        my_real, dimension(nel), intent(inout)         :: dplanl   !< Non-local plastic strain increment 
        my_real, dimension(nel), intent(in)            :: loff     !< Gauss point deletion status
        ...
        my_real, dimension(nel), intent(in)            :: shf      !< Transverse shear correction factor
        my_real, dimension(nel,3), intent(inout)       :: sigb     !< Backstress components
        integer, intent(in)                            :: l_dmg    !< Size of the damage array
        my_real, dimension(nel,l_dmg), intent(inout)   :: dmg      !< Damage array
        integer, intent(in)                            :: l_planl  !< Size of the non-local plastic strain array
        my_real, dimension(nel*l_planl), intent(in)    :: planl    !< Non-local plastic strain array
!-------------------------------------------------------------------------------
!  L o c a l   V a r i a b l e s
!-------------------------------------------------------------------------------      
        integer :: i,ii,nindx,indx(nel),ipos(nel,2),opte,vp,iter,nindxf,       &
           indxf(nel)
        my_real :: nu,c,a,h,p,m,epsmax,epsr1,epsr2,fisokin,einf,ce
        my_real :: young(nel),a11(nel),a12(nel),shear(nel)
        my_real :: dseq_dk1,dseq_dk2,dk1_dsigxx,dk1_dsigyy,dk2_dsigxx,         &
