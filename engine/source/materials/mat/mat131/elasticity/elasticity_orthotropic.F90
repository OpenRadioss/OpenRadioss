...
!||====================================================================
      subroutine elasticity_orthotropic(                                       &
...
!
        ...
        if (eltype == 1) then     
          ...
          if (matparam%ieos > 0) then
            soundsp(1:nel) = sqrt(abs(dpdm(1:nel) +                            &
              four_over_3*matparam%shear)/min(rho(1:nel),matparam%rho0))
          else
        ...
        elseif (eltype == 3) then
          !< Elastic stiffness matrix
          cstf(1:nel,1,1) = matparam%young
          cstf(1:nel,4,4) = matparam%shear*shf(1:nel)
...
#include "vectorize.inc"
        ...
        enddo
!
      end subroutine elasticity_orthotropic
      end module elasticity_orthotropic_mod
