...
!||====================================================================
      module elasticity_plas_isotropic_mod
! \brief Compute plastic strain dependent isotropic elastic stress for /MAT/LAW131
! \details Compute the elastic stress tensor using plastic strain dependent
...
!||====================================================================
        subroutine elasticity_plas_isotropic(                                  &
...
!----------------------------------------------------------------
          ...
          real(kind=WP), dimension(nel) :: shear,lam,aii,aij,young0,bulk,dedp
          real(kind=WP) :: nu,einf,ce
!===============================================================================
!
          !=======================================================================
          !< - Plastic strain dependent isotropic elasticity
          !=======================================================================
          !< Young modulus
          young(1:nel) = matparam%young
          nu           = matparam%nu
          offset_var   = matparam%iparam(5)
          einf         = matparam%uparam(1)
          ce           = matparam%uparam(2)
!
          !< Interpolation of Young modulus
          !  at current plastic strain for all elements
          xvec(1:nel,1) = pla(1:nel)
          if (matparam%table(1)%notable > 0) then
            call table_mat_vinterp(matparam%table(1),nel,nel,vartmp(1:nel,1),  &
              xvec(1:nel,1),young,dedp)
          else
            young(1:nel) = young(1:nel) - (young(1:nel) - einf)*               &
                                   (one - exp(-ce*pla(1:nel)))
          endif
!
...
!
          ...
          if (eltype == 1) then
            ...
            epsxxe(1:nel) = sigoxx(1:nel)/young0(1:nel) -                      &
                         nu*sigoyy(1:nel)/young0(1:nel) -                      &
                         nu*sigozz(1:nel)/young0(1:nel)
            epsyye(1:nel) = sigoyy(1:nel)/young0(1:nel) -                      &
                         nu*sigoxx(1:nel)/young0(1:nel) -                      &
                         nu*sigozz(1:nel)/young0(1:nel)
            epszze(1:nel) = sigozz(1:nel)/young0(1:nel) -                      &
                         nu*sigoxx(1:nel)/young0(1:nel) -                      &
                         nu*sigoyy(1:nel)/young0(1:nel)
            epsxye(1:nel) = sigoxy(1:nel)*two*(one + nu)/young0(1:nel)
            epsyze(1:nel) = sigoyz(1:nel)*two*(one + nu)/young0(1:nel)
            epszxe(1:nel) = sigozx(1:nel)*two*(one + nu)/young0(1:nel)
          ...
          elseif (eltype == 2) then
            ...
            epszze(1:nel) = zero
            epsxye(1:nel) = sigoxy(1:nel)*two*(one + nu)/young0(1:nel)
            epsyze(1:nel) = sigoyz(1:nel)*two*(one + nu)/                      &
                       max((young0(1:nel)*shf(1:nel)),em20)
            epszxe(1:nel) = sigozx(1:nel)*two*(one + nu)/                      &
                       max((young0(1:nel)*shf(1:nel)),em20)
            !< Elastic stiffness matrix
            ...
            s13(1:nel) = - nu / young(1:nel)
            s23(1:nel) = - nu / young(1:nel)
            s43(1:nel) = zero
            !< Sound speed
          ...
          elseif (eltype == 3) then
            ...
            epszze(1:nel) = zero
            epsyze(1:nel) = zero
            epsxye(1:nel) = sigoxy(1:nel)*two*(one + nu)/                      &
                            max((young0(1:nel)*shf(1:nel)),em20)
            epszxe(1:nel) = sigozx(1:nel)*two*(one + nu)/                      &
                            max((young0(1:nel)*shf(1:nel)),em20)
...
#include "vectorize.inc"
          ...
          enddo
!
        end subroutine elasticity_plas_isotropic
      end module elasticity_plas_isotropic_mod
