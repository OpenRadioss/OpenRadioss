...
# Build opt_flag, then coupling (coupling may append to opt_flag)
set(OPENMP "-fopenmp")
...
# Optional: coupling (preCICE / CWIPI)
include(${CMAKE_CURRENT_LIST_DIR}/common/coupling_gfortran.cmake)
...
set(strict      "-Werror=aliasing -Werror=unused-dummy-argument -Werror=do-subscript -Werror=array-bounds -Werror=tabs -Werror=surprising ${error_uninitialized}")
set(strict_wo_mpi "-Werror=aliasing ${unused_dummy_arg} -Werror=do-subscript -Werror=array-bounds -Werror=tabs -Werror=surprising ${error_uninitialized}")
set(ARCH_FLAGS "")

# Compiler flags (debug/analysis/asan/release)
...
set(F_O0_compiler_flags "-nostdinc -O0 ${opt_flag} -w ${portability} ${precision_flag} ${cppmach} ${cpprel} -DCPP_comp=f90 -ffixed-line-length-none ${mpi_flag} ${ADF} ${coupling}")
set(F_O1_compiler_flags "-nostdinc -O1 ${opt_flag} -w ${portability} ${precision_flag} ${cppmach} ${cpprel} -DCPP_comp=f90 -ffixed-line-length-none ${mpi_flag} ${ADF} ${coupling}")
set(F_O2_compiler_flags "-nostdinc -O2 ${opt_flag} -w ${portability} ${precision_flag} ${cppmach} ${cpprel} -DCPP_comp=f90 -ffixed-line-length-none ${mpi_flag} ${ADF} ${coupling}")
set(C_O1_compiler_flags "-O1 ${opt_flag} -w ${precision_flag} ${cppmach} ${cpprel} ${mpi_flag} ${ADF} ${coupling}")
